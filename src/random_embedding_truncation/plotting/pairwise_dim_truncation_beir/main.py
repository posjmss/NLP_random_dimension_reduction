...
def main() -> None:
    ...
    parser.add_argument(
        "--result-dir",
        type=str,
        default="./outputs/pairwise_dimension_reduction_beir/results",
    )
    parser.add_argument("--summary-dir", type=str, default="./output_summary")
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./figures/pairwise_dim_truncation_beir",
    )
    ...
    if not pairwise_paths:
        raise FileNotFoundError(f"No BEIR pairwise result JSON files found in {result_dir}")
