"""Plot one-dimension-drop MTEB scores against the full baseline.

This file was added to visualize which dropped dimensions improve, degrade, or
leave the selected MTEB metric unchanged.
"""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=str, default="./figures/dim_att_analysis_mteb")
    parser.add_argument("--metric", type=str, default=DEFAULT_METRIC)
    parser.add_argument(
        "--model",
        type=str,
        default=None,
        help="Optional output_name/model stem to plot, e.g. e5-large-v2.",
    )
    ...
    if not summary_paths:
        raise FileNotFoundError(f"No MTEB summary JSON files found in {summary_dir}")
    ...
    for summary_path in summary_paths:
        ...
        plot_model(
            ...
            baseline=baseline,
            metric=args.metric,
            output_path=output_dir / f"{model_name}__mteb.png",
